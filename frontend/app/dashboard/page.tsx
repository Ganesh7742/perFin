...
export default function DashboardPage() {
  ...
  const scoreColor = health_score.total >= 75 ? OLIVE : health_score.total >= 50 ? SAGE : DANGER;
  ...
  const label = { fontSize: 10, fontWeight: 700 as const, letterSpacing: '0.08em', textTransform: 'uppercase' as const, color: MUTED, display: 'flex', alignItems: 'center', gap: 6, marginBottom: 14 };
  const div1px = { height: 1, background: BORDER, margin: '0' } as const;
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(270px, 1fr))', gap: 12, marginBottom: 20 }}>
          ...
          <div style={{ ...card, padding: 20, borderLeft: `3px solid ${BORDER}` }}>
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 12 }}>
              <div>
                <div style={{ fontSize: 10, color: MUTED, letterSpacing: '0.07em', textTransform: 'uppercase', marginBottom: 3 }}>CIBIL Score</div>
                <div style={{ fontSize: 24, fontWeight: 700, color: DEEP }}>{cibil_advice?.score}</div>
              </div>
              <span style={{ fontSize: 10, fontWeight: 700, padding: '3px 10px', borderRadius: 4, background: 'rgba(143,158,88,0.15)', color: '#464646' }}>{cibil_advice?.status}</span>
            </div>
